...
export async function createOrderItems(orderItem: CreateOrderItem) {
  ...
  if (error)
    throw new Error(`Tạo chi tiết đơn hàng thất bại: ${error.message}`);
}
...
export async function deleteOrder(orderId: string) {
  ...
  if (deleteItemsError)
    throw new Error(
      `Xóa chi tiết đơn hàng thất bại: ${deleteItemsError.message}`
    );
  ...
  if (deleteOrderError)
    throw new Error(`Xóa đơn hàng thất bại: ${deleteOrderError.message}`);
}
...
export async function deleteOrderItem(itemId: string) {
  ...
  if (error)
    throw new Error(`Xóa chi tiết đơn hàng thất bại: ${error.message}`);
}
